...
public class UploadFile {
	...
	public void ServiceUpload(MultipartHttpServletRequest multi, QuestionReply qr) {
		...
		MultipartFile file = multi.getFile("aqi_imgSysName");
	...
	public boolean sponfileUp(MultipartHttpServletRequest multi, int bd_num) {
		System.out.println("fileUp");
		// 1.이클립스의 물리적 저장경로 찾기
		String root = multi.getSession().getServletContext().getRealPath("/");
		System.out.println("root=" + root2);
	...
	public boolean downloadWri(String fullPath, String mj_portf, HttpServletResponse resp) throws Exception {
		//실제 파일 있는지 확인
				boolean f = false;
				
				// 한글파일 깨짐 방지
				String downFile = URLEncoder.encode(mj_portf, "UTF-8");
				// 파일명 뒤에 이상한 문자가 붙는 경우 아래코드 해결
				// downFile=downFile.replaceAll("//+","");
				// 파일 객체 생성
				File file = new File(fullPath);
