...
public class Paging {
	...
	private String kind; // 게시판의 종류

	public Paging(int maxNum, int pageNum, int listCount, int pageCount, String boardName, String kind) {
		this.maxNum = maxNum;
	...
	public String makeHtmlPaging() {
		...
		return makeHtml(currentGroup, totalPage, boardName, kind);
	}

	private String makeHtml(int currentGroup, int totalPage, String boardName, String kind) {
		...
		if (start != 1) {
			sb.append("<a href='" + boardName + "?pageNum=" + (start - 1) + "&kind=" + kind + "'>");
			sb.append("[◀]");
			sb.append("</a>");
		}
		...
		for (int i = start; i <= end; i++) {
			if (pageNum != i) { // 현재 페이지가 아닌 경우 링크처리
				sb.append("<a href='" + boardName + "?pageNum=" + i + "&kind=" + kind + "'>");
				sb.append(" [ ");
				sb.append(i);
				sb.append(" ] ");
				sb.append("</a>");
		...
		if (end != totalPage) {
			sb.append("<a href='" + boardName + "?pageNum=" + (end + 1) + "&kind=" + kind + "'>");
			sb.append("[▶]");
			sb.append("</a>");
		}
		return sb.toString();
